...
//typeliste donne des Ids, compoundT donne ̩galement des Ids ce qui pourrait cr̩er un conflit dans la r̬gle declaration. Pour r̩soudre ce probl̬me, on cr̩̩ une r̬gle typedeclaration2 qui emp̻che de d̩clarer quelque chose sans avoir mis un type auparavant
typedeclaration2:'ARRAY' arraylist->^(ARRAY arraylist)
|procedure->procedure
;

switchdec
	:	'SWITCH' identifier ':=' switchlist->^(SWITCH ^(':=' identifier switchlist)) 
	;
...
//Gestion des expressions logiques avec priorit̩ des op̩rateurs
...
//Gestion des expressions arithm̩tiques et logiques avec priorit̩ des op̩rateurs
...
//Gestion des expressions arithm̩tiques (sans op̩rations logiques)
...
statement
	...
	|'FOR' simpleAr ':=' forlist 'DO' statement->^(FOR ^(':=' simpleAr ^(LISTFOR forlist)) ^(DO statement))
	|begin->begin
	|actualparametrepart
	;

memoire	:ID(->ID) {Idmemoire=$memoire.tree;} statementWithID-> statementWithID|
      INT(->INT){Idmemoire=$memoire.tree;} statementWithID-> statementWithID
	;
	
comment	:COMMENT2
	;
	
statementWithID	:	(->{Idmemoire})':=' simpleOp->^(':=' $statementWithID simpleOp)
	|	actualparametrepart2
	|(->{Idmemoire})'['expression (',' expression)*']' ':=' simpleOp->^(':=' ^(ARRAYACCESS  $statementWithID ^(ACCCESS expression+)) simpleOp)   
...
statementWithLabel	:	(->{Idmemoire}) label2->^(LABEL $statementWithLabel label2)
	...
	|(->{Idmemoire})'FOR' simpleAr ':=' forlist 'DO' statement->^(LABEL $statementWithLabel ^(FOR ^(':=' simpleAr forlist) ^(DO statement)))
	|(->{Idmemoire})begin->^(LABEL $statementWithLabel begin)
	|(->{Idmemoire})actualparametrepart2->actualparametrepart2
...
label2	:	ID(->ID) {Idmemoire=$label2.tree;} statementLabelLoop->statementLabelLoop
               |INT (->INT) {Idmemoire=$label2.tree;} statementLabelLoop->statementLabelLoop
	;
...
instructionsIfWithLabel	:	 ID (->ID) {Idmemoire=$instructionsIfWithLabel.tree;} expressionWithId->expressionWithId
        |INT (->INT) {Idmemoire=$instructionsIfWithLabel.tree;} expressionWithId->expressionWithId//cc
...
instructionsIf	:	  'FOR' simpleAr ':=' forlist 'DO' statement -> ^(FOR ^(':=' simpleAr forlist) ^(DO statement))
                  ...
                  |INT (->INT) {Idmemoire=$instructionsIf.tree;} instructionsIfWithID -> instructionsIfWithID 
                  |  gotostatement ->gotostatement 
                  |begin ->begin
                  |
	;
...
instructionsIfWithID	:	(->{Idmemoire})expressionWithId ->expressionWithId 
                |(->{Idmemoire})':' instructionsIfLabelLoop->instructionsIfLabelLoop
	;

expressionWithId	 :	(->{Idmemoire})':='  simpleOp ->^(':=' $expressionWithId simpleOp )
